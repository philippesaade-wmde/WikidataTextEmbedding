"""Create a self-contained HTML report from WikiProject statistics JSON."""
...
OUTPUT_PATH = os.environ.get(
    "HTML_OUTPUT_PATH", "data/pair_filter_matrix_report.html"
)


def build_html(stats):
    """Build an interactive, self-contained HTML report."""
...
<title>Wikidata WikiProject Filter Matrix</title>
<style>
:root {
  color-scheme: dark;
  --bg: #0b1020;
...
td { min-width: 30px; height: 30px; cursor: crosshair; transition: outline .1s ease; }
...
<body>
<main>
  <h1>Wikidata WikiProject filter matrix</h1>
  <p id="summary"></p>
  ...
  <div class="cards">
    <div class="card"><div class="label">Eligible entities</div><div class="value" id="total"></div></div>
    ...
    <div class="card"><div class="label">Excluded classes</div><div class="value" id="classes"></div></div>
  </div>
...
const friendly = name => name.replaceAll("_", " ");
const scopeSelect = document.querySelector("#scope");
const knowledgeTree = stats.knowledge_tree || {};

document.querySelector("#summary").textContent =
  `${stats.eligibility_note || "Explore filter coverage and pairwise intersections."} ` +
  `Knowledge-tree exclusion classes: ${formatter.format(knowledgeTree.class_count || 0)}.`;
...
function render() {
  ...
  document.querySelector("#classes").textContent = formatter.format(knowledgeTree.class_count || 0);
...
def main():
    """Read the JSON input and write the HTML report."""
    ...
    with open(OUTPUT_PATH, "w", encoding="utf-8") as output_file:
        output_file.write(build_html(stats))
